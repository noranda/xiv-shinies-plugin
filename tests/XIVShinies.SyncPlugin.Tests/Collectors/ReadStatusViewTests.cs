...
// separate groups so the window can label each. It is pure and Dalamud-free, so every line the panel can
// draw is pinned here rather than only seen in game — and, like CategorySettingsView, it must contain no
...
public class ReadStatusViewTests
{
    ...
    // test needs this: a manifest-driven collection's line is only dropped when there is a container
    // line left standing to report in its place.
    ...
    // container lines must have one of those collections switched on for them to belong to.
    private static CategorySettingsRow ManifestRow(string key = "items") =>
        Row(key, usesItemManifest: true);

    // A collection that was read this pass has nothing for the user to do, so it reads as healthy.
    [Fact]
    public void An_enabled_collection_that_was_read_reports_that_it_was_read()
    {
        var status = ReadStatusView.Build(new[] { Row("mounts") }, NoSources());

        var note = Assert.Single(status.Collections);
        Assert.Equal("mounts display: read.", note.Text);
    ...
    // the category's own display name. Nothing here names achievements.
    [Fact]
    ...
    // give. The line still has to appear — the collection genuinely did not make it into the upload —
    ...
    [Fact]
    public void A_collection_the_user_switched_off_gets_no_line_at_all()
    {
    ...
    [Fact]
    public void A_collection_the_server_switched_off_gets_no_line_at_all()
    {
    ...
    // A healthy manifest-driven collection would only say "read." — which the container lines beneath it
    // already say, in more detail, since the containers ARE where its items were counted. So it is
    // omitted rather than repeating them. The rule is keyed on the collector's own UsesItemManifest flag,
    // never on its category name.
    [Fact]
    public void A_manifest_driven_collection_that_was_read_gets_no_line_of_its_own()
    {
    ...
    // The suppression stands down when there are no container lines. Suppressing costs the reader
    // nothing only while a container line is there to report in the collection's place; when the
    // server's manifest is empty the pass opens no container and reports no source note at all, so
    // dropping the collection's line too would leave the panel silent about a collection the user has
    // switched on.
    [Fact]
    public void A_manifest_driven_collection_keeps_its_line_when_no_container_line_stands_in_for_it()
    {
        var status = ReadStatusView.Build(
            new[] { Row("items", usesItemManifest: true) }, NoSources());

        var note = Assert.Single(status.Collections);
        Assert.Equal("items display: read.", note.Text);
        Assert.Equal(SourceTone.Live, note.Tone);
    }

    // The suppression asks whether a container LINE exists, not whether a container status arrived: a
    // status SourceNoteText has no copy for is dropped from the panel, so it cannot stand in for
    // anything the reader can see.
    [Fact]
    public void A_manifest_driven_collection_keeps_its_line_when_every_container_status_was_dropped()
    {
        ...
        Assert.Equal("items display: read.", Assert.Single(status.Collections).Text);
    }

    // The other half of that rule: a manifest-driven collection that was SKIPPED still gets its line.
    // Why it was missed (the config has not arrived, the inventory is unreadable) exists nowhere else in
    // the panel — the container lines cannot say it.
    [Fact]
    public void A_manifest_driven_collection_that_was_skipped_still_reports_its_hint()
    {
        ...
        // With a container line present — the state that would suppress a HEALTHY manifest-driven
        // collection. A skipped one is owed its line regardless, because no container line explains why.
    ...
    // container lines standing in for it, so its healthy "read." line is the only thing that says the
    // sync can see it at all.
    [Fact]
    public void A_collection_that_is_not_manifest_driven_keeps_its_read_line()
    {
    ...
    // SourceNoteText returns null for a (source, state) pair it has no line for. A null means "nothing
    // worth saying", so the panel must drop it rather than print a raw wire string.
    [Fact]
    ...
    // collection line must never end up among the containers, or vice versa.
    [Fact]
    public void Collections_and_containers_are_returned_as_separate_groups()
    {
        ...
        Assert.Equal(new[] { "mounts display: read." }, status.Collections.Select(note => note.Text));
        Assert.Equal(new[] { "Inventory: read." }, status.Containers.Select(note => note.Text));
    }

    [Fact]
    public void Collection_lines_follow_the_order_of_the_rows_they_came_from()
    {
        var status = ReadStatusView.Build(new[] { Row("b"), Row("a") }, NoSources());

        Assert.Equal(
            new[] { "b display: read.", "a display: read." },
            status.Collections.Select(note => note.Text));
    }

    [Fact]
    public void No_rows_and_no_sources_produce_no_lines_in_either_group()
    {
    ...
    // THE GATE: a collection this plugin has never heard of gets a line built from its own row, proving
    // the panel holds no table of known category names.
    [Fact]
    public void A_collection_for_an_unknown_category_still_gets_a_line()
    {
        var status = ReadStatusView.Build(new[] { Row(UnknownCategory) }, NoSources());

        var note = Assert.Single(status.Collections);
        Assert.Equal("facewear display: read.", note.Text);
        Assert.Equal(SourceTone.Live, note.Tone);
    }
    ...
    // collection switched off, nothing is reading them — and a line urging the user to open their
    // saddlebag for a scan that is not happening is advice they cannot act on.
    [Fact]
    public void Container_lines_are_dropped_when_no_manifest_driven_collection_is_on()
    {
    ...
    // Its saddlebag and retainer lines have to go with it — they describe a scan that is not happening.
    [Fact]
    public void Container_lines_are_dropped_when_the_user_switches_the_manifest_collection_off()
    {
        var status = ReadStatusView.Build(
    ...
    [Fact]
    public void Container_lines_are_dropped_when_the_server_switches_the_manifest_collection_off()
    {
        var status = ReadStatusView.Build(
    ...
    // Switching the collection back on brings its containers back with it — the lines are dropped, not
    // forgotten.
    [Fact]
    public void Container_lines_return_once_a_manifest_driven_collection_is_on()
    {
        var status = ReadStatusView.Build(
